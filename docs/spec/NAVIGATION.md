...
| `/campaigns/:id/participants/:pid/atelier` | JWT | Atelier d'**un autre participant**, en lecture seule — vue maître-détail sur une seule page (colonne de véhicules + panneau de détail du véhicule sélectionné + synthèse de budget), aucune action de mutation. Accessible via l'entrée "Voir l'atelier" du menu ⋯ d'un autre participant sur `/campaigns/:id`, dès que la campagne n'est plus `EN_CONSTRUCTION` (mode campagne, **implémenté**, cf. [CAMPAIGN.md](CAMPAIGN.md#consultation-en-lecture-seule-de-latelier-dun-participant)) |
