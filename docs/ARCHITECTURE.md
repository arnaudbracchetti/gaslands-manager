...
### 8.3 E2E frontend — base de test dédiée et backend isolé
...
`apps/backend/.env` ni `app.module.ts`. `synchronize: true` (déjà actif, cf. §4) crée
alors le schéma automatiquement au premier démarrage.
