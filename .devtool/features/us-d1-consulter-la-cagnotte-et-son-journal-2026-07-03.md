...
- [x] Étant donné mon équipe en campagne, quand j'ouvre « Mon Atelier », alors la
...
Partiellement implémenté — le solde de cagnotte s'affiche désormais dans `AtelierPage` (colonne `atp-wallet`) et l'IHM complète de l'Atelier existe. Le 1ᵉʳ critère est coché. En revanche, le 2ᵉ critère (journal détaillé des transactions) reste non couvert par une vue dédiée — `GameJournalModal` expose les événements d'*une partie donnée* (via `GET .../games/:gameId/journal`), pas un historique consolidé de cagnotte par équipe. Reste à implémenter : endpoint de lecture de l'historique et UI dédiée.
